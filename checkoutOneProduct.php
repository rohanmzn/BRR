...
if (!isset($_SESSION['Username']) || $_SESSION['Username'] === '0') {
  include './product/invalidBuyer.php';
  exit;
}
...
<body>
  ...
  <div class="check-out">
    ...
    <table border=0>
      ...
      <div class="checkout-product">
        ...
        $sn = 1;
        $sum = 0;
        $sql1 = "SELECT product_name,price FROM products WHERE product_id = '$p_id'";
          ...
          <?php if ($result1->num_rows > 0) {
            while ($row1 = $result1->fetch_assoc()) { ?>
              ...
              <td style="width: 80%; text-align:left; padding-left:20px;">
                <?php echo $row1['product_name'] . ' (' . $usersize . ')'; ?>
              </td>
              <td style="width: 15%;">
                <?php echo $row1['price'];
                $sum = $sum + $row1['price'];
    ...
    <div class="payment">
      <table>
        <tr>
          <td>Delivery Address: <br><input type="text" id="deliveryAddressInput" value="<?php echo $userAddress ?>"
        ...
        <tr>
          <td>Add a note: <br><textarea id="myTextarea" rows="4" cols="50"
              placeholder="Add short note to seller reagarding delivery date and time and more...."></textarea></td>
        </tr>
        <tr>
    ...
    <form action="./myProfile/orderPlaced1.php" method="POST" id="orderForm">
  ...
  <script>
    ...
    var config = {
      ...
      "eventHandler": {
        onSuccess(payload) {
          ...
          console.log(payload);
          // Get the updated delivery address and note
          ...
          window.location.href = 'myProfile/orderPlaced1.php?pm=2&deliveryAddress=' + encodeURIComponent(updatedDeliveryAddress) + '&orderNote=' + encodeURIComponent(orderNote);
        },
        onError(error) {
          console.log(error);
        },
    ...
    btn.onclick = function () {
      ...
      checkout.show({ amount: <?php echo "200"; ?> * 100 });//ecjo $total sum gareko 200 mathi ko garna diyena
    }
